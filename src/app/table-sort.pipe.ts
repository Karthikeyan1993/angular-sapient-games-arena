...
export class TableSortPipe implements PipeTransform {
  transform(value: any[], prop?: string, orderBy?: string): any {
    ...
    const _orderBy = orderBy.toLocaleLowerCase().toString();
    const _prop = prop.toLocaleLowerCase().toString();
    const _temp = _orderBy == "asc" ? -1 : 1;

    const _value = value.sort((a, b) => {
      const _a = isArrayOfObj ? a[_prop] : a;
      const _b = isArrayOfObj ? b[_prop] : b;
      if (typeof _a == "string") {
